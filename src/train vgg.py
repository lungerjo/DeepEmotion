...
def main(cfg: DictConfig) -> None:
    ...
    subset_size = 10  # Number of samples to use
    train_dataloader = torch.utils.data.DataLoader(
        torch.utils.data.Subset(train_dataloader.dataset, range(subset_size)),
        batch_size=cfg.train.batch_size, shuffle=True
    )
    val_dataloader = torch.utils.data.DataLoader(
        torch.utils.data.Subset(val_dataloader.dataset, range(subset_size)),
