...
class StudyForrestVolumeIndexer:
    def __init__(self, cfg: DictConfig):
        ...
        if self.label_mode == "classification":
            self.labels_df = self.classification_labels
        elif self.label_mode == "soft_classification":
            self.labels_df = self.soft_classification_labels
        else:
        ...
        self.index_mappings = self._create_index_mappings() # lst[{'aligned_label_idx': int, 'data_file_idx': int, 'row_idx': int}]
    ...
    def _create_index_mappings(self):
        ...
        for session_idx in range(len(self.session_offsets)):
            ...
            session_mask = (
                (self.labels_df['offset'] >= session_start) &
                (self.labels_df['offset'] < session_end)
            )
            session_labels = self.labels_df[session_mask]

            for row_idx, label_row in session_labels.iterrows():
                offset = int(label_row['offset'])
...
class ZarrDataset(Dataset):
    def __init__(self, zarr_path: str, label_mode: str, debug: bool, cfg: DictConfig = None):
        ...
        if self.aligned_labels_csv:
            ...
            raise ValueError("Missing aligned_labels_csv in Zarr store attributes.")
    ...
    def __getitem__(self, idx: int):
        ...
        volume_idx, local_idx = self.valid_indices[idx]
        volume_idx = int(volume_idx)
        local_idx = int(local_idx)
        
        
        # Extract the data slice
        data_slice = self.data[volume_idx, :, :, :, local_idx]
        data_tensor = torch.from_numpy(data_slice)

        # Extract the label
        if self.label_mode == "classification":
            label = int(self.labels[volume_idx, local_idx])
            label_tensor = torch.tensor(label, dtype=torch.long)
        elif self.label_mode == "soft_classification":
            label = self.labels[volume_idx, local_idx, :]
        ...
        if self.aligned_labels is not None:
            subset = self.aligned_labels[
                (self.aligned_labels["vol_idx"] == volume_idx)
                & (self.aligned_labels["local_idx"] == local_idx)
            ]
            if subset.empty:
                print(f"[ERROR] No aligned label found for volume_idx={volume_idx}, local_idx={local_idx}")
                ...
                raise IndexError(f"Missing global_idx for volume_idx={volume_idx}, row_idx={local_idx}")
